    ...
    public class ItemMasterLovRepository
    {
        ...
        public List<LovModel> itemmaster_lov_role_get(string auth_username)
        {
            try
            {
                ...
                Connection();
                VSK_Data.Open();
                List<LovModel> RequestModelList = SqlMapper.Query<LovModel>(VSK_Data, "SP_ItemMaster_Lov_Role_Get", objParam, commandType: CommandType.StoredProcedure, commandTimeout: 600).ToList();
                VSK_Data.Close();
                return RequestModelList.ToList();
            }
            catch (Exception ex)
